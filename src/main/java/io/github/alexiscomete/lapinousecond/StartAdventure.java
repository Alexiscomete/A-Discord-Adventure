            ...
            if (messageCreateEvent.isPrivateMessage() || messageCreateEvent.getServer().get().getId() != 854288660147994634L) {
                ...
                if (msga.isPresent()) {
                    ...
                    try {
                        ...
                        Thread.sleep(3000);
                        user.sendMessage("Le principe est simple : il existe une histoire principale commune à tout les serveurs, mais chaque serveur peut aussi avoir sa propre histoire plus ou moins configurable ! Vous pouvez voyager de serveur en serveur quand le bot vous envoie une invitation, le plus souvent après avoir acheté par exemple un ticket pour voyager sur un bateau !");
                        Thread.sleep(3000);
                        user.sendMessage("Vous pouvez choisir de limiter le type de serveur que le bot vous envoie : officiel (comprenant des serveurs normaux mais aussi des serveurs event et passage obligatoire), vérifié régulièrement, vérifié, normal, tous. Nous vous conseillons de mettre normal ou vérifié, pour une meilleur expérience de jeu.");
                        Thread.sleep(3000);
                        user.sendMessage("Veuillez envoyer votre choix avec la commande `sec` : 1 - tous, 2 - normal, 3 - vérifié, 4 - vérifié régulièrement, 5 - officiel");
