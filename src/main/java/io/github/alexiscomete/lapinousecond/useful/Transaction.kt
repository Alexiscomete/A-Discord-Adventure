...
open class Transaction(
    ...
    fun make(textChannel: TextChannel, quantity: Double, player: Player) {
        make(
            quantity,
            player,
            { content: String? -> textChannel.sendMessage(content) }) { content: String? ->
    ...
    fun make(quantity: Double, player: Player, acc: Consumer<String?>, de: Consumer<String?>) {
        if (basic(quantity)) {
            acc.accept(player.getAnswer(AnswerEnum.TR_END, true))
        } else {
            de.accept(
                player.getAnswer(
                    AnswerEnum.ECHEC_TRANS,
                    true,
                    player.getAnswer(AnswerEnum.NO_ENOUGH_MONEY, false)
                )
            )
        }
    }
